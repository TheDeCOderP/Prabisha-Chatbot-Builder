...
export async function POST(request: NextRequest) {
  try {
    ...
    const defaultFormFields = [
      { id: "1", type: "TEXT" as const, label: "Full Name", required: true, placeholder: "Enter your full name", order: 0 },
      { id: "2", type: "EMAIL" as const, label: "Email Address", required: true, placeholder: "Enter your email address", order: 1 },
      { id: "3", type: "PHONE" as const, label: "Phone Number", required: false, placeholder: "Enter your phone number", order: 2 },
      { id: "4", type: "TEXT" as const, label: "Company", required: false, placeholder: "Enter your company name", order: 3 }
    ];

    const defaultTriggers = [
      { type: "KEYWORD" as const, keywords: ["help", "support", "contact"], position: 0 },
      { type: "END_OF_CONVERSATION" as const, position: 1 }
    ];
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      return { chatbot, form: chatbotForm, logic: chatbotLogic };
    });
